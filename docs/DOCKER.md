...
docker build \
  ...
  -t meysam8498/daftaram:2.6.0 \
  -t meysam8498/daftaram:latest \
  .

# بررسی تصویر
docker images | grep daftaram
docker inspect meysam8498/daftaram:2.6.0 --format '{{.Config.Healthcheck}}'
```
...
services:
  web:
    image: ghcr.io/meysam8498/daftaram:2.6.0   # یا ساخت محلی با build
    build:
      context: .
      args:
...
# تگ‌گذاری
docker tag meysam8498/daftaram:2.6.0 ghcr.io/meysam8498/daftaram:2.6.0
docker tag meysam8498/daftaram:2.6.0 ghcr.io/meysam8498/daftaram:latest

# انتشار
docker push ghcr.io/meysam8498/daftaram:2.6.0
...
```bash
docker login
docker tag meysam8498/daftaram:2.6.0 meysam8498/daftaram:2.6.0
docker push meysam8498/daftaram:2.6.0
```

برای استفاده از داکر هاب در `docker-compose.yml` مقدار `image` را به
`meysam8498/daftaram:2.6.0` تغییر دهید.
...
docker pull ghcr.io/meysam8498/daftaram:2.6.0
docker rm -f daftar-web
docker run -d --name daftar-web -p 8080:80 \
  --restart unless-stopped \
  ghcr.io/meysam8498/daftaram:2.6.0
